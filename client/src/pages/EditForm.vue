<template>
  <div
    class="h-screen w-full flex flex-col bg-[#F8FAFF] font-inter overflow-hidden"
  >
    <!-- ================= TOP TOOLBAR ================= -->
    <header
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          <div
            class="w-6 h-6 bg-[#5855F6] rounded-md flex items-center justify-center"
          >
            <svg
              width="12"
              height="12"
      ...
      <div class="flex items-center gap-4">
        <span
          v-if="successMessage"
          class="text-xs font-bold text-theme-success flex items-center gap-1"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-4 w-4"
        ...
        <router-link
          v-if="currentFormId && formLoaded"
          :to="`/preview/f/${currentFormId}`"
          target="_blank"
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <aside
        class="w-64 bg-white border-r border-theme-border flex-shrink-0 overflow-y-auto p-5"
      >
        <div class="mb-8">
          <label
            class="block text-xs font-bold text-theme-muted uppercase tracking-wider mb-3"
      ...
      <main
        class="flex-1 overflow-y-auto p-8 flex justify-center scroll-smooth"
      >
        <!-- Loading State -->
        <div
          v-if="isLoading"
        ...
        >
          <svg
            class="animate-spin h-8 w-8 text-theme-primary mb-4"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
          >
        ...
        >
          ...
          <div class="flex-1 p-10 pt-6 space-y-3">
            ...
            >
              <!-- Drag Handle Icon -->
              <div
                class="cursor-grab active:cursor-grabbing text-gray-300 group-hover:text-gray-500 p-1 -ml-2"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
              ...
              <div class="flex-1 flex flex-col gap-1.5 pointer-events-none">
                <label class="text-sm font-bold text-theme-text">{{
                  field.label || "Untitled Field"
                }}</label>
                <div
                  class="px-3 py-2 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-400 truncate"
                >
                  {{
                    field.type === "checkbox"
                      ? "Check to agree"
        ...
        <div v-else class="text-center py-20 m-auto">
          ...
          <router-link
            to="/dashboard"
            class="px-6 py-2 bg-theme-primary text-white font-bold rounded-lg shadow-sm"
            >Return to Hub</router-link
          >
        </div>
      </main>
      ...
      <aside
        class="w-72 bg-white border-l border-theme-border flex-shrink-0 overflow-y-auto"
      >
        <div v-if="activeField" class="p-5">
          <label
            class="block text-xs font-bold text-theme-muted uppercase tracking-wider mb-5 flex items-center gap-2"
          >
            <!-- Dynamic Icon in properties sidebar too -->
            <span
              v-html="getFieldIcon(activeField.type)"
              class="text-theme-primary w-4 h-4"
            ></span>
          ...
          <div class="space-y-5">
            ...
            <div class="relative">
              ...
              <div
                ...
                :class="
                  isTypeDropdownOpen ? 'ring-2 ring-theme-primary bg-white' : ''
                "
              >
              ...
              >
                <div v-for="group in fieldTypes" :key="group.group">
                  <div
                    class="px-3 py-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider bg-gray-50/80"
                  >
                    {{ group.group }}
                  </div>
                  ...
                  >
                    <!-- Icon render -->
                    <span
                      v-html="getFieldIcon(item.value)"
                      class="w-4 h-4"
                      :class="
                        activeField.type === item.value
                          ? 'text-theme-primary'
                    ...
                    <svg
                      v-if="activeField.type === item.value"
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
                      height="14"
                      viewBox="0 0 24 24"
                      ...
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      class="ml-auto text-theme-primary"
                    >
          ...
          >
            <label class="block text-[11px] font-bold text-gray-500 mb-3"
...
const onDrop = (targetIndex) => {
  ...
  if (activeFieldIndex.value === draggedIndex.value) {
    ...
    if (
      draggedIndex.value < activeFieldIndex.value &&
      targetIndex >= activeFieldIndex.value
    ) {
      activeFieldIndex.value--;
    } else if (
      draggedIndex.value > activeFieldIndex.value &&

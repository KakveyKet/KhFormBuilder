  ...
  <div class="gen-layout">
    <div class="gen-grid-container">
      ...
      <aside class="gen-sidebar-left">
        <transition enter-active-class="transition duration-500 ease-out" enter-from-class="opacity-0 -translate-x-4" enter-to-class="opacity-100 translate-x-0">
          <div v-if="generatedFields?.length > 0 && !isLoading" class="gen-sidebar-card">
            <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-[#5855F6]"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/></svg>
              Appearance
            </h3>
            ...
            <div class="flex flex-col gap-2">
              ...
              >
                ...
                <div class="flex-shrink-0 flex gap-0.5">
                  <div class="w-3.5 h-3.5 rounded-full border border-gray-200 shadow-inner" :style="{ backgroundColor: tpl.colors.primary }"></div>
                  <div class="w-3.5 h-3.5 rounded-full border border-gray-200 shadow-inner" :style="{ backgroundColor: tpl.colors.bg }"></div>
                </div>
                <span class="text-sm font-bold truncate" :class="selectedTemplate === tpl.id ? 'text-[#5855F6]' : 'text-gray-700'">
                  {{ tpl.name }}
                </span>
                
                <svg v-if="selectedTemplate === tpl.id" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="ml-auto text-[#5855F6] flex-shrink-0"><polyline points="20 6 9 17 4 12"></polyline></svg>
              </button>
            </div>
          </div>
        </transition>
      ...
      <main class="gen-main-center">
        ...
        <transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 -translate-y-4" enter-to-class="opacity-100 translate-y-0">
          <div v-if="!currentUser" class="gen-login-warning">
            <div class="flex items-center gap-3">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="text-orange-500">
        ...
        <div v-if="!prediction && !isLoading" class="gen-header flex flex-col items-center text-center">
          ...
          <p class="gen-subtitle">Describe your ideal data collection form in plain English or Khmer, and our AI will generate it instantly.</p>
        </div>
        ...
        <div class="gen-prompt-card" :class="{ 'opacity-50 pointer-events-none': !currentUser || isLoading }">
          ...
          <div class="gen-prompt-footer">
            <div class="flex items-center gap-3">
              ...
              <span v-if="isRecording" class="text-xs font-bold text-red-500 animate-pulse flex items-center gap-1.5 uppercase tracking-widest hidden sm:flex">
                <span class="w-2 h-2 rounded-full bg-red-500"></span> Recording
              </span>
            </div>

            <button @click="predictMask" :disabled="isLoading || !currentUser || !inputText.trim()" class="gen-submit-btn group">
              ...
              <span v-else class="flex items-center gap-2">
                Generate Form
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="transition-transform group-hover:translate-x-1 group-disabled:translate-x-0"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
              </span>
            </button>
          </div>
        </div>
        ...
        <div v-if="!prediction && !isLoading" class="gen-suggestions animate-fade-up">
          ...
          <div class="flex flex-wrap justify-center gap-2">
            <button @click="fillSuggestion('A simple contact form with name, email, and a message box.')" class="gen-suggestion-pill">Contact Form</button>
            <button @click="fillSuggestion('Job application form asking for resume, experience, and role.')" class="gen-suggestion-pill">Job Application</button>
            <button @click="fillSuggestion('សូមបង្កើតទម្រង់អាពាហ៍ពិពាហ៍ឱ្យខ្ញុំ')" class="gen-suggestion-pill">Wedding RSVP (Khmer)</button>
          </div>
        </div>

        <!-- Error State -->
        <div v-if="errorMessage" class="gen-error-banner animate-fade-up">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="flex-shrink-0"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
        ...
        <div v-if="generatedFields?.length > 0 && !isLoading" class="gen-browser-window animate-fade-up" :style="activeThemeStyle">
          <div class="gen-browser-header">
            ...
            <div class="flex-1 text-center flex justify-center">
              <span class="text-xs font-mono text-gray-400 bg-white px-3 py-1 rounded-md border border-gray-200 truncate max-w-[150px] sm:max-w-xs">
          ...
          <form @submit.prevent class="p-6 sm:p-10 pointer-events-none bg-[var(--tpl-bg)] text-[var(--tpl-text)] border-x border-b border-[var(--tpl-border)] rounded-b-2xl relative transition-colors duration-500">
            <!-- Header -->
            <div class="mb-8 pb-6 border-b border-[var(--tpl-border)] flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              ...
              <div class="flex items-center justify-center sm:justify-end gap-2 hidden sm:flex" style="color: var(--tpl-primary)">
                <svg width="24" height="24" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="32" height="32" rx="8" fill="currentColor" /><circle cx="16" cy="16" r="6" fill="var(--tpl-bg)" /></svg>
            ...
            <div class="space-y-5">
              <div v-for="field in generatedFields" :key="field.id" class="flex flex-col gap-1.5">
                ...
                <!-- Standard Inputs -->
                <input v-if="['text', 'email', 'number', 'password', 'date'].includes(field.type)" :type="field.type" :placeholder="field.placeholder" class="w-full px-4 py-2.5 border rounded-lg text-sm outline-none transition-colors" style="background-color: color-mix(in srgb, var(--tpl-text) 3%, var(--tpl-bg)); border-color: var(--tpl-border); color: var(--tpl-text);" />

                <!-- Textarea -->
                <textarea v-else-if="field.type === 'textarea'" :placeholder="field.placeholder" rows="3" class="w-full px-4 py-2.5 border rounded-lg text-sm outline-none transition-colors resize-none" style="background-color: color-mix(in srgb, var(--tpl-text) 3%, var(--tpl-bg)); border-color: var(--tpl-border); color: var(--tpl-text);"></textarea>

                <!-- Select -->
                <select v-else-if="field.type === 'select'" class="w-full px-4 py-2.5 border rounded-lg text-sm outline-none transition-colors appearance-none" style="background-color: color-mix(in srgb, var(--tpl-text) 3%, var(--tpl-bg)); border-color: var(--tpl-border); color: var(--tpl-text);">
                ...
                <div v-else-if="field.type === 'checkbox'" class="flex items-center gap-3 mt-1">
                  <input type="checkbox" class="w-5 h-5 rounded border" style="accent-color: var(--tpl-primary); border-color: var(--tpl-border);" />
                ...
                <div v-else-if="field.type === 'radio'" class="flex flex-col gap-3 mt-1">
                  <label v-for="option in field.options" :key="option" class="flex items-center gap-3">
                    <input type="radio" :name="'radio-' + field.id" :value="option" class="w-5 h-5 border" style="accent-color: var(--tpl-primary); border-color: var(--tpl-border);" />
                    <span class="text-sm font-medium">{{ option }}</span>
                  </label>
                </div>

                <!-- Fallback -->
                <input v-else type="text" :placeholder="field.placeholder" class="w-full px-4 py-2.5 border rounded-lg text-sm outline-none transition-colors" style="background-color: color-mix(in srgb, var(--tpl-text) 3%, var(--tpl-bg)); border-color: var(--tpl-border); color: var(--tpl-text);" />
              </div>
            </div>

            <!-- Submit Button -->
            <button type="submit" class="w-full mt-8 py-3.5 font-bold rounded-xl shadow-md transition-transform hover:-translate-y-0.5" style="background-color: var(--tpl-primary); color: #ffffff">
              Submit Form
            </button>
          </form>
        </div>
        ...
        <transition enter-active-class="transition duration-500 ease-out" enter-from-class="opacity-0 translate-x-4" enter-to-class="opacity-100 translate-x-0">
          <div v-if="generatedFields?.length > 0 && !isLoading" class="flex flex-col gap-6">
            ...
            <div class="gen-sidebar-card flex flex-col items-center text-center py-6">
              <div class="w-14 h-14 rounded-full flex items-center justify-center mb-3 transition-colors" :class="isPublished ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'">
                <svg v-if="isPublished" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><polyline points="20 6 9 17 4 12" /></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" /></svg>
              </div>
            ...
            <div class="gen-sidebar-card flex flex-col gap-3">
              ...
              <button v-if="!isPublished" @click="publishForm" :disabled="isToggling" class="btn-publish-shortcut w-full">
                ...
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" /></svg>
...
const changeTheme = async (templateId) => {
  ...
  try {
    await axios.put(`http://localhost:3000/api/forms/${formId.value}`, { template_id: templateId });
  } catch (err) {

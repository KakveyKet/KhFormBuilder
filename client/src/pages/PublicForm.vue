<template>
    ...
    <div class="min-h-screen py-12 px-4 sm:px-6 flex justify-center items-start transition-colors duration-500 form-wrapper-bg"
        :style="activeThemeStyle">

        <div v-if="isLoading" class="flex flex-col items-center justify-center pt-32">
            <svg class="animate-spin h-10 w-10 mb-4" style="color: var(--tpl-primary);"
        ...
        <div v-else-if="errorMessage" class="max-w-xl w-full p-10 rounded-2xl shadow-xl text-center mt-20"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto mb-4 opacity-50"
                style="color: var(--tpl-text);" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div v-else-if="isSubmitted"
            class="max-w-xl w-full p-12 rounded-2xl shadow-2xl text-center mt-20 animate-fade-up"
            style="background-color: var(--tpl-bg); border: 1px solid var(--tpl-border);">
            ...
            <div class="p-8 sm:p-12">
                ...
                <div v-if="formData.header_file && formData.header_file.url" class="mb-8 flex"
                    ...
                    <img v-if="formData.header_file.type === 'image'" :src="formData.header_file.url"
                        class="max-h-32 object-contain rounded-lg shadow-sm border"
                ...
                <form ref="formRef" @submit.prevent="submitForm" class="space-y-8">
                    ...
                    <div v-for="field in formSteps[currentStep].fields" :key="field.id" class="flex flex-col">
                        ...
                        <input v-if="['text', 'email', 'number', 'password', 'url', 'tel'].includes(field.type)"
                            :type="field.type" :placeholder="field.placeholder" v-model="answers[field.id]"
                            :required="field.required" class="form-input-base" />

                        <!-- 2. Date & Time Inputs -->
                        <div v-else-if="['date', 'time', 'datetime-local'].includes(field.type)"
                            class="form-date-wrapper">
                            <div
                                class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none transition-colors form-date-icon">
                                <svg v-if="field.type === 'date' || field.type === 'datetime-local'"
                        ...
                        <textarea v-else-if="field.type === 'textarea'" :placeholder="field.placeholder"
                            v-model="answers[field.id]" :required="field.required" rows="4"
                            class="form-input-base resize-y"></textarea>
                        ...
                        <div v-else-if="field.type === 'radio'" class="flex flex-col gap-2 mt-2">
                            <label v-for="opt in field.options" :key="opt" class="form-radio-card"
                                ...
                                <input type="radio" class="sr-only" :name="'radio-' + field.id" :value="opt"
                                    v-model="answers[field.id]" :required="field.required" />
                        ...
                        <label v-else-if="field.type === 'checkbox'" class="form-checkbox-wrapper">
                            <div class="flex items-center h-6">
                                <input type="checkbox" class="form-checkbox-input peer sr-only"
                                    v-model="answers[field.id]" :required="field.required" />
                                <div class="form-checkbox-box">
                                    <svg class="form-checkbox-icon" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div v-else-if="field.type === 'file'">
                            <label class="form-dropzone" :class="{ 'is-active': answers[field.id] }">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4">
                                    <svg v-if="!answers[field.id]" class="form-dropzone-icon-empty"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                    </svg>
                                    <svg v-else class="form-dropzone-icon-filled" xmlns="http://www.w3.org/2000/svg"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>

                                    <p class="mb-1 text-sm font-bold truncate max-w-full"
                                        style="color: var(--tpl-text);">
                                        <span v-if="answers[field.id]">{{ answers[field.id] }}</span>
                                        <span v-else>Click to upload file</span>
                                    </p>
                                    <p v-if="!answers[field.id]" class="text-xs"
                                        style="color: var(--tpl-text); opacity: 0.6;">Any document or image</p>
                                </div>
                    ...
                    <div class="flex items-center gap-4 mt-10 border-t pt-8" style="border-color: var(--tpl-border);">
                        <button v-if="currentStep > 0" type="button" @click="prevStep" class="form-btn-nav"
                            title="Previous Step">
                        ...
                        <button v-else type="submit" :disabled="isSubmitting" class="form-btn-primary">
                            <svg v-if="isSubmitting" class="animate-spin h-6 w-6" xmlns="http://www.w3.org/2000/svg"

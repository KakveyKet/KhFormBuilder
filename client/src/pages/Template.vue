<template>
  <div class="min-h-screen bg-[#F8FAFF] flex flex-col">
    ...
    <section class="w-full pt-16 pb-12 px-5 flex flex-col items-center text-center">
      <div
        class="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 text-[#5855F6] text-xs font-bold uppercase tracking-widest rounded-full mb-6">
      ...
      <p class="text-lg text-gray-500 max-w-2xl leading-relaxed mb-10">
        Choose from our expertly crafted templates or let the AI select the best design for your use case. <strong>All
          themes are completely free to use.</strong>
      </p>
    </section>
    ...
    <main class="flex-1 w-full max-w-[1200px] mx-auto px-5 pb-24">
      ...
      <div v-if="isLoading" class="flex flex-col items-center justify-center py-20">
        <svg class="animate-spin h-10 w-10 text-[#5855F6] mb-4" xmlns="http://www.w3.org/2000/svg" fill="none"
          viewBox="0 0 24 24">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div v-for="tpl in templates" :key="tpl.id"
          ...
          <!-- "Free" Badge -->
          <div
            class="absolute top-4 right-4 z-10 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider backdrop-blur-md border shadow-sm bg-white/80 text-gray-600 border-gray-200">
            Free
          </div>
          ...
          <div class="h-56 w-full relative flex items-center justify-center p-6"
            ...
            <!-- The Mock Form Card -->
            <div
              class="w-full max-w-[240px] rounded-xl shadow-lg border p-5 transition-transform duration-500 group-hover:scale-105"
              :style="{ backgroundColor: tpl.colors?.bg || '#fff', borderColor: tpl.colors?.border || '#eee', fontFamily: tpl.font === 'custom' ? 'Inter' : tpl.font }">

              <!-- Mock Title -->
              <div class="h-3 w-2/3 rounded mb-4"
                :style="{ backgroundColor: tpl.colors?.text || '#333', opacity: 0.9 }"></div>

              <!-- Mock Inputs -->
              <div class="space-y-3 mb-5">
                <div class="h-8 w-full border rounded-md"
                  :style="{ borderColor: tpl.colors?.border || '#eee', backgroundColor: `color-mix(in srgb, ${tpl.colors?.text || '#000'} 3%, ${tpl.colors?.bg || '#fff'})` }">
                </div>
                <div class="h-8 w-full border rounded-md"
                  :style="{ borderColor: tpl.colors?.border || '#eee', backgroundColor: `color-mix(in srgb, ${tpl.colors?.text || '#000'} 3%, ${tpl.colors?.bg || '#fff'})` }">
                </div>
            ...
            <div class="absolute inset-0 opacity-10 pointer-events-none"
              :style="{ background: `radial-gradient(circle at 80% 20%, ${tpl.colors?.primary || '#5855F6'} 0%, transparent 50%)` }">
            </div>
          </div>
...
// Hardcoded robust fallback list to show all beautiful themes to everyone (All Free now!)
const fallbackTemplates = [
  ...
  { id: "dark-knight", name: "Dark Knight", requiredPlan: "free", colors: { primary: "#818CF8", bg: "#0F172A", text: "#F8FAFC", border: "#334155" }, font: "Inter", designStyle: "dark-mode" },
  { id: "elegant-serif", name: "Elegant Serif", requiredPlan: "free", colors: { primary: "#0F172A", bg: "#FAFAFA", text: "#171717", border: "#E5E5E5" }, font: "Lora", designStyle: "professional" },
  { id: "neon-cyber", name: "Neon Cyber", requiredPlan: "free", colors: { primary: "#22D3EE", bg: "#171717", text: "#FFFFFF", border: "#22D3EE" }, font: "Roboto", designStyle: "modern" },
  { id: "corporate-slate", name: "Corporate Slate", requiredPlan: "free", colors: { primary: "#334155", bg: "#E2E8F0", text: "#0F172A", border: "#94A3B8" }, font: "Inter", designStyle: "corporate" },
  { id: "midnight-purple", name: "Midnight Purple", requiredPlan: "free", colors: { primary: "#C084FC", bg: "#2E1065", text: "#FAF5FF", border: "#5B21B6" }, font: "Inter", designStyle: "vibrant" },

  { id: "enterprise-gold", name: "Enterprise Gold", requiredPlan: "free", colors: { primary: "#EAB308", bg: "#000000", text: "#FFFFFF", border: "#CA8A04" }, font: "Inter", designStyle: "premium" },
  { id: "custom-brand-kit", name: "Custom Brand Kit", requiredPlan: "free", colors: { primary: "#111827", bg: "#FFFFFF", text: "#111827", border: "#D1D5DB" }, font: "Inter", designStyle: "clean" }
];
...
onMounted(async () => {
  try {
    ...
    if (response.data && Array.isArray(response.data) && response.data.length > 0) {
      ...
      templates.value = response.data.map(t => ({ ...t, requiredPlan: 'free' }));
    } else {
...
// Relies on the color rather than the plan now
const getMockupBg = (tpl) => {
  if (tpl.colors?.bg === '#0F172A' || tpl.colors?.bg === '#171717') return '#1E293B';
  if (tpl.colors?.bg === '#000000') return '#111111';
  return '#F3F4F6';
};
</script>